...
export const createNewEditTemplate = (event) => {
  const {schedule, offers, info} = event;

  const formattedStartDate = getFormattedDate(schedule.start);
  const formattedEndDate = getFormattedDate(schedule.end);
